###THESE ARE MY CURRENT QUESTIONS TO ADDRESS FOR THIS PROGRAM###
# How can it create a saveable and returnable file
# How to define a phone number as either cell, home, or work
# How to edit contacts or get options 


# importing the module sys and csv 
import csv 
import sys
...
def initial_phonebook():
    ...
    for i in range(rows):
        ...
        # such as name, number, e-mail id, dob, category etc
            ...
            if j == 3:
                temp.append(
                    str(input("Enter a number type(Home/Cell/Work/Other): ")))
                # Even if this field is left as blank, None will take the blank's place
                if temp[j] == "" or temp[j] == ' ':
                    temp[j] = None
...
def add_contact(pb):
    ...
    for i in range(len(pb[0])):
        ...
        if i == 3:
            dip.append(
                str(input("Enter a number type(Home/Cell/Work): ")))      
    pb.append(dip)
...
def remove_existing(pb):
    ...
    if temp == 0:
        ...
        print("Sorry, you have entered an invalid query.\n  Please re-check and try again later.")
...
def search_existing(pb):
    ...
    choice = int(input("\nEnter search criteria\n\n\n1. Name\n2. Number\n3. Email-id\n4. Enter a number type(Home/Cell/Work):\n\n Please enter: "))
    ...
    # This will execute for searches based on contact category
        query = str(
            input("Please enter the category number type of the contact you wish to search: "))
        for i in range(len(pb)):
            if query == pb[i][4]:
                check = i
                temp.append(pb[i])
...
# A simple gesture of courtesy towards the user to enhance user experience
    print("Thank you for using Logans Smartphone directory system.")
    print("Please visit again!")
    sys.exit("Goodbye, and have a nice day!")

def saveContacts (sContacts):

    header = ['Name', 'Number', 'E-mail']
    ...
    with open ('Phone_List.csv', 'w') as f:
        ...
        #write multiple rows
        writer.writerows(sContacts)
        
def loadContacts():
    contacts = []
    try:
        with open ('Phone_List.csv', 'r') as f:
             reader = csv.reader(f)
